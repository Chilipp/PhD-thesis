...
psyplot has been developed as a side project within the PhD-Thesis \citep{Sommer2020e} at the University of Lausanne, Switzerland. Most of the text in this document has been extracted from this chapter. psyplot is now further maintained and developed by Philipp S. Sommer at the Helmholtz-Zentrum Hereon.
...
psy-view \citep{Sommer2021} is a relatively new interface build upon the psyplot framework for a quick visualization of netCDF files. Being inspired by ncview\footnote{\url{http://meteora.ucsd.edu/~pierce/ncview_home_page.html}}, it runs as a stand-alone software, but additionally as a plugin for the general psyplot \gls{gui}. psy-view provides quick functionalities to open datasets, select and visualize four-dimensional variables, change the dimensions such as time or vertical level, and update the appearance of the plot, e.g. the projection, title or color coding.
